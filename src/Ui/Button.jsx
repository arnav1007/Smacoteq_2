...
function Button({ children, disabled, to, type, onClick }) {
  ...
  const base =
    "inline-block font-roboto rounded-full bg-main text-stone-200 font-semibold uppercase tracking-wide  transition-colors duration-300 hover:bg-stone-700 focus:bg-stone-500 focus:outline-none focus:ring focus:ring-stone-900 focus:ring-offset-2 disabled:cursor-not-allowed text-sm";
  ...
  if (to) {
    return (
      <Link to={to} className={styles[type]} target="_blank">
        {children}
      </Link>
    );
  }
